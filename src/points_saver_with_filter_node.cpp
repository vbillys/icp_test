...
void savePointCloudWithFilter (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
  static int n=0;
  //VPointCloud::Ptr cloud(new VPointCloud());
  //PointXYZ::Ptr cloud(new PointXYZ());
  ...
  pcl::fromROSMsg(PointMatcher_ros::pointMatcherCloudToRosMsg<float>(mapPointCloud, "ibeo", ros::Time::now()), *cloud_filtered);
  transferPclPointCloudToXYPointsMap(cloud, &g_m2);
  /*char buffer[100];*/
  snprintf(buffer, sizeof(buffer), "scan_filtered_%d.txt", n++);
  ok_is_saved = g_m2.save2D_to_text_file(std::string(buffer));
  std::cout << buffer << " " << ok_is_saved << std::endl;
}
...
{
  ros::init(argc, argv, "points_saver_with_filter_node");
  ros::NodeHandle nh;//("~");
  ros::Rate r(20);
  ...
  ros::Subscriber sub = nh.subscribe ("ibeo_points", 1 , savePointCloudWithFilter);
