...
def update_odometry(_Tleft, _Tright):
    ...
    global tot_d_left_wheel 
    global tot_d_right_wheel 
    global tot_d_center 
    ...
    tot_d_left_wheel = tot_d_left_wheel + d_left_wheel
    tot_d_right_wheel = tot_d_right_wheel + d_right_wheel
    tot_d_center = tot_d_center + d_center
...
rospy.init_node('enco_A9_to_ros',anonymous=False)
pub_odom = rospy.Publisher('encoder_odom',Odometry)
...
while True:
    try:
        ...
        if len(_data) == 32:  # ecoms packed capture
                ...
                odom.pose.pose.position.x = prev_x
                odom.pose.pose.position.y = prev_y
                odom.pose.pose.position.z = prev_head

                odom.twist.twist.angular.x = vl
                odom.twist.twist.angular.y = vr

                odom.twist.twist.linear.x = tot_d_left_wheel
                odom.twist.twist.linear.y = tot_d_right_wheel
                odom.twist.twist.linear.z = tot_d_center
